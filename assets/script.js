...
function sendBooking(serviceName, isTyres){
  ...
  if(!name || !phone || !reg){
    alert("Please complete your name, phone number and vehicle registration.");
    return;
  }
  ...
  if(isTyres){
    ...
    if(!tyreQuantity || !tyreSize || !tyreType || !tyreRange){
      alert("Please complete all tyre details.");
      return;
    }

    text +=
      "Number of tyres: " + tyreQuantity + "\n" +
      "Tyre size: " + tyreSize + "\n" +
      "Tyre type: " + tyreType + "\n" +
